...
function regexpToText({ locale, text }) {
  ...
  const rawTags = regexpToArray(`(?:(\\()|(\\))|(log2|log10|clz32|log1p|\\£|\\$|\\€|[a-z]+)(?:\\^(\\d+))?|(\\%)|(0x[0-9a-f]+(?:\\.[0-9a-f]+)?|0o[0-7]+(?:.[0-7]+)?|0b[01]+(?:\\.[01]+)?|${numberFormat})|(#[0-9a-f]{3}(?:[0-9a-f]{5}|[0-9a-f]{3}|[0-9a-f])?)|(\\*\\*|[=+\\-*\\/^])|(,))`, lowerText);
